...
static void ramfs_lookup(fuse_req_t req, fuse_ino_t parent, const char *name) {
  ...
  if (auto ind = dynamic_pointer_cast<INodeDir>(INode::find(parent))) {
    if (auto in = ind->find(name)) {
      struct fuse_entry_param e {};
      e.ino = in->m_inode;
      e.attr_timeout = 1.0;
      e.entry_timeout = 1.0;
      ramfs_stat(e.ino, &e.attr);
...
static void ramfs_readdir(fuse_req_t req, fuse_ino_t ino, size_t size,
  ...
  if (auto inDir = dynamic_pointer_cast<INodeDir>(INode::find(ino))) {
    struct dirbuf b {};
...
static void ramfs_read(fuse_req_t req, fuse_ino_t ino, size_t size, off_t off,
                       [[maybe_unused]] struct fuse_file_info *fi) {
  TRACE(ino)
  if (auto in = dynamic_pointer_cast<INodeFile>(INode::find(ino)))
...
static void ramfs_create(fuse_req_t req, fuse_ino_t parent, const char *name,
  ...
  TRACE(parent)
  fuse_entry_param ep{};
  if (auto in = dynamic_pointer_cast<INodeDir>(INode::find(parent))) {
    auto newin = in->addFile(name, mode);
    ep.ino = newin->m_inode;
    ramfs_stat(ep.ino, &ep.attr);
...
static void ramfs_rename(fuse_req_t req, fuse_ino_t parent, const char *name,
  ...
  auto indold = dynamic_pointer_cast<INodeDir>(INode::find(parent));
  auto indnew = dynamic_pointer_cast<INodeDir>(INode::find(newparent));
...
void ramfs_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name,
  ...
  if (auto parentin = dynamic_pointer_cast<INodeDir>(INode::find(parent))) {
...
void ramfs_listxattr(fuse_req_t req, fuse_ino_t ino, size_t size) {
  TRACE(ino);
  fuse_reply_err(req, ENOSYS);
}
...
int mount(struct fuse_args args, const char *mountpoint) {
  ...
  ramfs_oper.listxattr = ramfs_listxattr;
